...
layui.use(['form','jquery',"layer"],function() {
    ...
    if(window.sessionStorage.getItem('userFace') &&  $(".").length > 0){
        $("#userFace").attr("src",window.sessionStorage.getItem('userFace'));
        $(".").attr("src",$(".").attr("src").split("images/")[0] + "images/" + window.sessionStorage.getItem('userFace').split("images/")[1]);
    }else{
        $("#userFace").attr("src","${pageContext.request.contextPath }/resources/images/face1.jpg");
    }
    ...
    function lockPage(){
        layer.open({
            ...
            content : '<div class="admin-header-lock" id="lock-box">'+
                            '<div class="admin-header-lock-img"><img src="images/face1.jpg" class=""/></div>'+
                            '<div class="admin-header-lock-name" id="lockUserName">${user.user.name}</div>'+
                            '<div class="input_btn">'+
            ...
            success : function(){
                //判断是否设置过头像，如果设置过则修改顶部、左侧和个人资料中的头像，否则使用默认头像
                if(window.sessionStorage.getItem('userFace') &&  $(".").length > 0){
                    $(".").attr("src",$(".userAvatar").attr("src").split("images/")[0] + "images/" + window.sessionStorage.getItem('userFace').split("images/")[1]);
                }
            }
        })
